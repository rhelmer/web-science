...
(async function () {
  ...
  const privateWindowResults = await browser.storage.local.get(
    "webScience.socialMediaLinkSharing.privateWindows"
  );
  if (
    "webScience.socialMediaLinkSharing.privateWindows" in
      privateWindowResults &&
  ...
  function logReshareClick(clicked) {
    ...
    for (const post of posts) {
      if (post.contains(mostRecentReshare)) {
        ...
        fetch(profile.getAttribute("href"), { credentials: "omit" }).then(
          (rFF) => {
            rFF.text().then((text) => {
  ...
  function reshareSourceTracking() {
    const reshareButtons = document.querySelectorAll(
  ...
  function recStructure(node) {
    ...
    if (node.textContent == "") {
      links = node.querySelectorAll
        ...
        : [];
      links = Array.prototype.map.call(links, (link) =>
        link.href ? removeShim(link.href).url : null
      );
  ...
  function isComments(structure) {
    ...
    if (structure.length >= 2) {
      if (
        "text" in structure[0] &&
        structure[0].text == "Like" &&
        "text" in structure[1] &&
        structure[1].text == "Comment"
      ) {
        return true;
      }
    }
  ...
  browser.runtime.onMessage.addListener(async (request) => {
    return new Promise((resolve, reject) => {
      ...
      const requestedPost = document.body.querySelector(
        `a[href*="${request.postId}"]`
      );
