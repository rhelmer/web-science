...
(async function () {
  ...
  function isElementVisible(element) {
    ...
    if (opacity === "" || opacity === "0") return false;
    return true;
  }
  ...
  function timerTick() {
    ...
    forEachAnchorElement((anchorElement) => {
      ...
      if (linkInfo === undefined) {
        ...
        if (
          elementRect.width < linkMinimumWidth ||
    ...
    if (
      !pageManager.pageHasAttention &&
      (lastLostAttention < 0 ||
        lastLostAttention + attentionIdlePeriod < timeStamp)
    ) {
   ...
   */
  const observer = new IntersectionObserver(anchorObserverCallback, {
    threshold: linkMinimumVisibility,
  });
  ...
  if ("webScience" in window && "pageManager" in window.webScience)
    ...
    if (!("pageManagerHasLoaded" in window)) window.pageManagerHasLoaded = [];
