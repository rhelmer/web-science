...
const pageNavigation = function () {
  ...
  function updateMaxRelativeScrollDepth() {
     ...
     */
    if (
      !pageManager.pageHasAttention ||
      (!scrollDepthWaitForAttention &&
        Date.now() - pageManager.pageVisitStartTime < scrollDepthUpdateDelay) ||
      (scrollDepthWaitForAttention &&
        (firstAttentionTime <= 0 ||
          Date.now() - firstAttentionTime < scrollDepthUpdateDelay)) ||
      Math.max(document.documentElement.offsetHeight, window.scrollY) <
        scrollDepthMinimumHeight
    ) {
      return;
    }
  ...
  function pageVisitStart({ timeStamp }) {
    ...
    if (pageManager.pageHasAttention) {
      scrollDepthIntervalId = setInterval(
        updateMaxRelativeScrollDepth,
  ...
  pageManager.onPageVisitStop.addListener(({ timeStamp }) => {
    ...
    if (pageManager.pageHasAttention && pageManager.pageHasAudio)
      attentionAndAudioDuration +=
        timeStamp - Math.max(lastAttentionUpdateTime, lastAudioUpdateTime);
