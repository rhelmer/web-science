...
(function () {
  ...
  pageManager.sendMessage = function (message) {
    try {
      browser.runtime.sendMessage(message).catch((reason) => {
        debugLog(
          `Error when sending message from content script to background page: ${JSON.stringify(
            message
          )}`
        );
      });
  ...
  function pageVisitStart(timeStamp, isHistoryChange = false) {
    ...
    pageManager.pageHasAttention = isHistoryChange
      ? pageManager.pageHasAttention
      : false;
    pageManager.pageHasAudio = isHistoryChange
      ? pageManager.pageHasAudio
      : false;
  ...
  browser.runtime.onMessage.addListener((message) => {
    ...
    // page visit start
    if (
      message.type === "webScience.pageManager.urlChanged" &&
