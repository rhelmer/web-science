...
class Event {
  ...
  notifyListeners(listenerArguments = []) {
    this.listeners.forEach((options, listener) => {
      try {
        if (
          this.notifyListenersCallback === null ||
          this.notifyListenersCallback(listener, listenerArguments, options)
        )
...
class EventSingleton extends Event {
  ...
  addListener(listener, options) {
    if (this.listeners.size > 0)
      throw new Error(
        "Error: cannot add more than one listener to EventSingleton event."
      );
