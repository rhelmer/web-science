...
async function initialize() {
  ...
  currentIdleState = await browser.idle.queryState(
    minimumIdleStateDetectionIntervalInSeconds
  );
  if (currentIdleState === "idle")
    lastIdleTime =
      Date.now() - minimumIdleStateDetectionIntervalInSeconds * 1000;
...
function idleOnStateChangedListener(newState) {
  ...
  for (const [
    ...
    idleStateListenersWithDetectionInterval,
  ] of idleStateListeners)
    scheduleIdleStateTimeout(
      idleStateListenersWithDetectionInterval,
      detectionIntervalInSeconds
    );
}
 ...
 * @private
 */
function scheduleIdleStateTimeout(
  idleStateListenersWithDetectionInterval,
  detectionIntervalInSeconds
) {
  ...
  const delayTime = Math.max(
    lastIdleTime + detectionIntervalInSeconds * 1000 - Date.now(),
    0
  );
...
async function addListener(idleStateListener, detectionIntervalInSeconds) {
  ...
  // and we're done
  let idleStateListenersWithDetectionInterval = idleStateListeners.get(
    detectionIntervalInSeconds
  );
  ...
  idleStateListenersWithDetectionInterval = idleStateListeners.set(
    detectionIntervalInSeconds,
    new Set().add(idleStateListener)
  );
  ...
  if (
    ...
    Date.now() < lastIdleTime + detectionIntervalInSeconds * 1000
  )
    scheduleIdleStateTimeout(
      idleStateListenersWithDetectionInterval,
      detectionIntervalInSeconds
...
async function removeListener(idleStateListener, detectionIntervalInSeconds) {
  const idleStateListenersWithDetectionInterval = idleStateListeners.get(
    detectionIntervalInSeconds
  );
  if (idleStateListenersWithDetectionInterval !== undefined) {
    // Remove the listener
    ...
    if (idleStateListenersWithDetectionInterval.size === 0) {
      ...
      const idleStateTimeoutID = idleStateTimeouts.get(
        detectionIntervalInSeconds
      );
